                    ...
                    <TableBody>
                        {query.data?.data.data.map((entry: any) => (
                            <TableRow key={entry.id}>
                                ...
                                <TableCell>{entry.distance_clip}</TableCell>
    ...
    async function saveAdditionalPrice() {
        if (!modalState.data.id) {
            const _data_to_add = {
                ...
                distance_clip: modalState.data.distance_clip,
            }
            await api.post('master/dap-master/', _data_to_add)
            ...
            await api.patch('master/dap-master/', _data_to_update)
            setModalState({ isOpen: false, data: null })
    ...
    return <div className="absolute bg-slate-500/30 top-0 p-2 left-0 h-screen w-screen flex justify-center items-center">
        <div className="rounded-md bg-white max-w-md w-full shadow p-4 flex flex-col">
            ...
            <div className="flex-grow overflow-visible">
                ...
                <div className="grid w-full items-center gap-1.5 p-1">
                    <Label htmlFor="additional-distance">Base Distance</Label>
                    <Input value={modalState.data?.distance_clip} onChange={e => {
                        setModalState({
                            ...modalState,
                            data: {
                                ...modalState?.data ?? {},
                                distance_clip: e.target.value
                            }
                        })
...
function HistoryModal({ id, closeHistoryModal }: { id: number, closeHistoryModal: Function }) {
    ...
    const query = useQuery({ queryKey: ['audit-history', 'DAP', id], queryFn: getAdditionalPriceHistory })
