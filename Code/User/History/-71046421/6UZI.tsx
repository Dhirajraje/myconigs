...
async function getAdditionalPrices() {
    return await api.get('master/dap-master/')
}

export default function AdditionalPriceConfig() {
    const query = useQuery({ queryKey: ['additional-prices'], queryFn: getAdditionalPrices })
    ...
    return <div className="flex flex-col gap-2 w-full max-w-7xl p-2">
        {
            query.isLoading ? <div className="p-10 flex w-full justify-center items-center">
                ...
                <div className="flex gap-2 items-center">
                    <h2 className="text-lg font-semibold flex-grow">Additional Prices</h2>
                    <Button onClick={_ => {
                        setModalState({
                            isOpen: true,
                            data: null
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead>Additional Distance</TableHead>
                    ...
                    <TableBody>
                        {query.data?.data.data.map((entry: any) => (
                            <TableRow key={entry.id}>
                                ...
                                <TableCell>{entry.additional_price}</TableCell>
                                <TableCell>{entry.additional_distance}</TableCell>
...
function Modal({ setModalState, modalState }: { setModalState: Function, modalState: { isOpen: boolean, data: any } }) {
    ...
    async function saveAdditionalPrice() {
        if (!modalState.data.id) {
            const _data_to_add = {
                additional_price: modalState.data.additional_price,
                additional_distance: modalState.data.additional_distance,
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({ queryKey: ['additional-prices'] })
        },
    })
        ...
        <div className="rounded-md bg-white max-w-md w-full shadow p-4 flex flex-col">
            ...
            <div className="flex-grow overflow-visible">
                <div className="grid w-full items-center gap-1.5 p-1">
                    <Label htmlFor="additional-price">Additional Price</Label>
                    <Input value={modalState.data?.additional_price} onChange={e => {
                        setModalState({
                            ...modalState,
                            data: {
                                ...modalState.data ?? {},
                    ...
                    }} type="number" id="additional-price" placeholder="Additional Price" />

                </div>
                <div className="grid w-full items-center gap-1.5 p-1">
                    <Label htmlFor="additional-distance">Additional Distance</Label>
                    <Input value={modalState.data?.additional_distance} onChange={e => {
                        setModalState({
                            ...modalState,
                            data: {
                                ...modalState?.data ?? {},
                                additional_distance: e.target.value
                            }
                        })
                    }} type="number" id="additional-distance" placeholder="Additional Distance" />

                </div>
                <div className="grid w-full items-center gap-1.5 p-1">
                    <Label htmlFor="">Additional Distance</Label>
                    <FancyMultiSelect options={dropdownItems}
                        ...
                        onSelectChange={(values: any) => {
                            setModalState({
                                ...
                                data: {
                                    ...modalState?.data ?? {},
                                    applicable_days: values.map((el:any) => el.value)
                                }
                            })
...
function HistoryModal({ id, closeHistoryModal }: { id: number, closeHistoryModal: Function }) {
    ...
    const query = useQuery({ queryKey: ['audit-history', 'BDP', id], queryFn: getAdditionalPriceHistory })
