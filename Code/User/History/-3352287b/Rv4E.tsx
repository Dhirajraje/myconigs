            ...
            <div className="p-2 flex gap-2 bg-slate-300 w-min rounded-lg">
                {
                    navigations.map((el,index)=>{
                        return <NavLink key={index} to={el.path} className={({isActive})=>`p-2 font-bold whitespace-nowrap rounded-sm ${isActive?" bg-white shadow ring-indigo-600 ring-2 ring-offset-2":" "}`}>{el.name}</NavLink>
                    })
                }
            </div>
        </div>
