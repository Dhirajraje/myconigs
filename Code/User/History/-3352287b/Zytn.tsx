...
export default function HomeWrap() {
    ...
    return <div className="w-screen h-screen flex flex-col">
        <div className="max-w-7xl mx-auto w-full py-2">
            <div className="p-2 flex gap-2 bg-slate-300 w-min rounded-md">
                {
                    navigations.map((el,index)=>{
                        return <NavLink key={index} to={el.path} className={({isActive})=>`p-2 font-bold whitespace-nowrap ${isActive?" bg-white":" bg-slate-50"}`}>{el.name}</NavLink>
                    })
                }
            </div>
        </div>
