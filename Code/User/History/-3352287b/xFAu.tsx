            ...
            <div className="p-2 flex gap-2 bg-slate-300 w-min rounded-lg">
                {
                    navigations.map((el,index)=>{
                        return <NavLink key={index} to={el.path} className={({isActive})=>`p-2 font-bold whitespace-nowrap rounded-md ${isActive?" bg-white shadow border-indigo-600 border-2":" "}`}>{el.name}</NavLink>
                    })
                }
            </div>
        </div>
