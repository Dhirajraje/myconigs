...
class CalculationView(APIView):
    ...
    def post(self,request):
        try:
            ...
            # Calculate base price and substract base distance for additional cost calculation
            _total_price = _request.data['base_price']
            _total_dist_travelled = _request.data['distance_travelled']
            if _dbp_data['base_distance'] <= _total_dist_travelled:
            ...
            if _total_dist_travelled != 0:
                ...
                # from distance a to b additional price will be x
                # from distance b to c additional price will be y
                # hence from distance a to c addional price will be x + y
                if len(_dap_values)>0:
                    prev_dist = 0
                    for dap in _dap_values:
                        _distance_applicable = dap.distance_clip - prev_dist
                        prev_dist = dap.distance_clip
                        if _total_dist_travelled <= _distance_applicable:
                            _total_price += dap.additional_price * _total_dist_travelled
                            _total_dist_travelled = 0
                            break
                        else:
                            _total_price = dap.additional_price * _distance_applicable
                            _total_dist_travelled -= _distance_applicable
            ...
            _tmf_values = TMFMaster.objects.order_by('time_clip').all()
            
            if _tmf_ceil_value is not None:
                _tmf_values = TMFMaster.objects.filter(time_clip__lte=_tmf_ceil_value.time_clip)

            _wc_ceil_value = WCMaster.objects.filter(time_clip__gte=_request.data['waiting_time']).order_by('-time_clip').first()
            
            _wc_values = WCMaster.objects.order_by('time_clip').all()      
            if _wc_ceil_value is not None:
                _wc_values = WCMaster.objects.filter(time_clip__lte=_wc_ceil_value.time_clip)
            
            print(
                [
                    _dbp_data,_dap_values,_tmf_values,_wc_values
                ]
            )
            
            print(_request.data['distance_travelled'])
